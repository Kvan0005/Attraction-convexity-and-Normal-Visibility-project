...
let polygon = []; // Sommets du polygone
...
const s = (p) => {
    ...
    p.draw = function () {
        ...
        p.beginShape();
        let totalSegments = polygon.length - 1; // Nombre total de segments
        ...
        for (let i = 0; i <= currentSegment; i++) {
            p.vertex(polygon[i].x, polygon[i].y);
        }

        // Ajouter un point intermédiaire pour le segment en cours
        if (currentSegment < totalSegments) {
            let start = polygon[currentSegment];
            let end = polygon[currentSegment + 1];
